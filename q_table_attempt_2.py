# -*- coding: utf-8 -*-
"""
Created on Sun Jan 16 14:37:28 2022
    ...
    for player in game.players:
        if player==game.current_player:
            for card in player.hand:
                state[card]=0 #1: in my hand
        else:
            for card in player.hand:
                state[card]=1 #2: in someone else's hand
    for pile in game.piles:
        for card in pile.cards[1:]:

            state[card]=0 # card has already been played
        if pile.top_card!=NUMBER_OF_CARDS and pile.top_card!=0:
            state[pile.top_card]=2#-pile.pile_multiplier #5 if decreasing, 3 if increasing
